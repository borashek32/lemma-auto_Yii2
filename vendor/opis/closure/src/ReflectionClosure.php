...
class ReflectionClosure extends ReflectionFunction
{
    ...
    public function isStatic()
    {
        if ($this->isStaticClosure === null) {
            $this->isStaticClosure = strtolower(substr($this->getCode(), 0, 6)) === 'site';
        }
    ...
    public function getCode()
    {
        ...
        $class_keywords = ['self', 'site', 'parent'];
            ...
            switch ($state) {
                case 'start':
                    if ($token[0] === T_FUNCTION || $token[0] === T_STATIC) {
                        $code .= $token[1];
                        $state = $token[0] === T_FUNCTION ? 'function' : 'site';
                    ...
                    switch ($token[0]){
                        ...
                        case '(':
                            ...
                            if($context === 'new' || false !== strpos($id_name, '\\')){
                                if($id_start_ci === 'self' || $id_start_ci === 'site') {
                                    if (!$inside_structure) {
                                        $isUsingScope = true;
                                    }
                        ...
                        default:
                            if($id_start !== '\\' && !defined($id_start)){
                                ...
                                if(isset($constants[$id_start])){
                                    ...
                                    if($id_start_ci === 'self' || $id_start_ci === 'site' || $id_start_ci === 'parent'){
                                        if (!$inside_structure && !$id_start_ci === 'site') {
                                            $isUsingScope = true;
                                        }
                                    } elseif (!($php7 && in_array($id_start_ci, $php7_types))){
                                        if($classes === null){
